...
Subject / fold : S24  (fold-22 of repro_focal_g3p0_effective_num_37)
                 → best LOSOCV subject (acc 0.923 / MCC 0.854 / bal-acc 0.938 / AUC 1.00)
Checkpoint     : output/checkpoints/repro_focal_g3p0_effective_num_37/
                 repro_focal_g3p0_effective_num_37_fold22_e{0..4}.pt   (5-member ensemble)
...
exact preprocessed tensors fed to the model:
  ...
  C4 EEG embedding     — fusion pre-hook args[0] (graph-GAT + LIF branch)
  C5 Graph embedding   — fusion pre-hook args[1] (dynamic connectivity branch)
  C6 ET embedding      — fusion pre-hook args[2] (eye-tracking attention branch)
...
ENSEMBLE      = sorted(CKPT_DIR.glob(f"{FOLD_TAG}_fold22_e*.pt"))
...
OUT = PHASE8 / "figures" / "case_study_S24"
for p in ["A", "B", "C", "D"]:
...
# ════════════════════════════════════════════════════════════════════════════
log("=" * 70)
log(" INS-HDGS-CMT — REAL case-study panels for S24 (fold-22, best subject)")
log("=" * 70)
...
pred      = (mean_prob >= 0.38).astype(int)        # opt_threshold for fold22 S24 (from CSV)
...
# capture the per-modality embeddings fed INTO the fusion module (eeg / graph / et)
mod_emb = {}
def _fusion_pre_hook(mod, args, kwargs):
    if len(args) >= 2:
        mod_emb["eeg"]   = args[0].detach()[0].cpu().numpy() if args[0] is not None else None
        mod_emb["graph"] = args[1].detach()[0].cpu().numpy() if args[1] is not None else None
        mod_emb["et"]    = (args[2].detach()[0].cpu().numpy()
                            if len(args) >= 3 and args[2] is not None else None)
hp = members[0].fusion.register_forward_pre_hook(_fusion_pre_hook, with_kwargs=True)
...
try:
    ...
    h.remove()
    hp.remove()
log(f"    modality embeddings captured: "
    f"{[k for k, v in mod_emb.items() if v is not None]}")
...
def panel_C():
    ...
    # ── C4/C5/C6: per-modality embeddings fed into the fusion transformer ────
    def plot_embed(vec, fname, title):
        if vec is None:
            log(f"    {title} skipped — embedding is None")
            return
        v = np.asarray(vec).ravel()
        Dv = v.size
        s = int(np.ceil(np.sqrt(Dv)))
        p = np.full(s * s, np.nan); p[:Dv] = v
        fig, ax = plt.subplots(figsize=(5.6, 5.2))
        im = ax.imshow(p.reshape(s, s), cmap="coolwarm")
        ax.set_title(f"{title} ({Dv}-d) — {SUBJECT} epoch {REP}")
        ax.set_xticks([]); ax.set_yticks([])
        fig.colorbar(im, ax=ax, fraction=0.046, label="activation")
        fig.tight_layout(); fig.savefig(OUT / fname); plt.close(fig)

    plot_embed(mod_emb.get("eeg"), "C/C4_eeg_embedding.png",
               "C4 · EEG embedding (graph-GAT + LIF)")
    plot_embed(mod_emb.get("graph"), "C/C5_graph_embedding.png",
               "C5 · Graph embedding (dynamic connectivity)")
    plot_embed(mod_emb.get("et"), "C/C6_et_embedding.png",
               "C6 · ET embedding (eye-tracking attention)")
    log("    Panel C done.")
...
if __name__ == "__main__":
    ...
    # provenance record
    rec = dict(subject=SUBJECT, fold="fold-22", experiment=FOLD_TAG,
